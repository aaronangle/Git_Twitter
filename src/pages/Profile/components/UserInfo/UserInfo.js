  ...
  return (
    <>
      <div>
        ...
        <img src={user.avatar_url} alt="" className={styles.banner__profile} />
      </div>
      <div className={styles.cont}>
        <h2 className="mb-0">{user.name}</h2>
        <p className="text--muted m-0">@{user.login}</p>
        <p>{user.bio}</p>
        <div className={styles.cont__details}>
          {user.blog && (
            <a
              href={user.blog}
              target="_blank"
              rel="noopener noreferrer"
              className={joinClassNames('fc-row', 'text--primary', styles.details__text)}
            >
              <svg
          ...
          {user.company && (
            <p className={joinClassNames('fc-row', 'text--muted', styles.details__text)}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
