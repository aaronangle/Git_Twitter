  ...
  return (
    <div className="">
      <p className="text--primary hover fs-row mb-0 " onClick={(e) => handleCommitClick(e)}>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" className={joinClassNames(styles.svg, showCommits && styles['svg--rotate'])}>
          ...
          {commits.map((commit, index) => {
            return (
              <p className={styles['commit']} key={index}>
                <span className={styles['commit__span']}>{commit.author.name}</span> - {commit.message}
              </p>
            );
          })}
        </div>
